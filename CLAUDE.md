# 단기과열종목 계산 서비스 개발 가이드

## 프로젝트 개요

단기과열종목 계산 서비스는 국내 주식시장에서 단기과열종목 예고에 걸린 종목이 실제로 단기과열종목에 지정되기 위한 조건을 계산해주는 웹 애플리케이션입니다.

### 핵심 기능

1. **종목 검색 및 데이터 조회**: 한국 주식 종목 코드 입력 시 KOSPI/KOSDAQ 자동 판별 및 Yahoo Finance API를 통한 실시간 데이터 조회
2. **단기과열종목 지정 조건 계산**: 주가상승률, 거래회전율, 주가변동성 조건 분석
3. **필요 수치 계산**: 조건 달성을 위한 필요 종가, 최소 거래량, 주가변동성 제시
4. **데이터 시각화**: 40영업일 주가 추이 차트 및 통계 대시보드 제공

### 기술 스택

- **프론트엔드**: Next.js 15.4.6 (App Router), TypeScript, Tailwind CSS 4.1.12
- **UI 컴포넌트**: shadcn/ui, Lucide React
- **차트**: Recharts
- **백엔드**: Next.js API Routes, Yahoo Finance API
- **아키텍처**: Turborepo 모노레포, pnpm 패키지 관리
...
- **영업일 필터링**: 주말 및 공휴일 제외한 40영업일 데이터 사용
...
- 모바일 우선 반응형 디자인
- 완전한 TypeScript 지원으로 타입 안전성 확보
- Yahoo Finance API 제한 고려 (향후 KRX Open API로 마이그레이션)
